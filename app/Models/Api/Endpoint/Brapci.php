...
class Brapci extends Model
{
    ...
    function avaliations()
    {
        ...
        $type = [];
        $fields = '*';
        $fields = 'period_start, period_end, name, evaluation_area, rdf_id, title, stratum, numeric_value';
    ...
    function get($v, $id = 0)
    {
        ...
        $ccUse = $dt['concept']['cc_use'] ?? $dt['concept']['id_cc'] ?? null;
        $ccId = $dt['concept']['id_cc'] ?? null;

        if (isset($dt['concept']['cc_status']) && ($ccUse !== null) && ($ccUse != $ccId))
            {
                $id = $ccUse;
                $dt = $RDF->le($id);
            }
